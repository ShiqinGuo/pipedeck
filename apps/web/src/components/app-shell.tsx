...
const NAV_ITEMS = [
  ...
  { to: '/projects', label: '仓库', icon: GitPullRequestArrow },
  { to: '/runs', label: '运行', icon: Activity },
  { to: '/workspaces', label: '工作区', icon: Workflow },
  { to: '/resources', label: '资源', icon: Container },
...
const ROUTE_TITLES: { prefix: string; title: string }[] = [
  { prefix: '/pipelines/', title: '管道预览' },
  { prefix: '/runs/', title: '运行详情' },
  { prefix: '/workspaces/', title: '工作区详情' },
  { prefix: '/projects', title: '仓库' },
  { prefix: '/pipelines', title: '管道' },
  { prefix: '/runs', title: '运行记录' },
  { prefix: '/workspaces', title: '工作区' },
  { prefix: '/resources', title: '本地资源' },
    ...
    <header className="flex h-13 min-h-13 shrink-0 items-center justify-between gap-4 border-b border-border bg-[#28272e] px-3 md:px-4">
      ...
      <div className="flex min-w-0 items-center gap-2.5">
        ...
        >
          ...
          <span>{token.loading ? '读取权限' : token.configured ? '本地写入' : '只读'}</span>
        </span>
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        ...
        >
          ...
          <span className="hidden text-[10px] md:hidden">{label}</span>
        </Link>
      </TooltipTrigger>
    ...
    <TooltipProvider>
      <div className="flex h-dvh flex-col md:grid md:grid-cols-[60px_minmax(0,1fr)]">
        ...
        <aside className="hidden flex-col items-center border-r border-[#3f3c46] bg-rail md:flex">
          ...
          <nav aria-label="主导航" className="grid w-full gap-1 p-2">
