...
![Pipedeck 功能动画：仓库组成工作区，依赖依次就绪，再从页面保存并读回数据](docs/media/demo.gif)

*程序绘制的功能演绎，使用示例数据；不是实际界面录屏。展示工作区编排、依赖就绪与应用读写链路。[静态图](docs/media/demo-poster.png)*
...
|---|---|---|
| 先看懂用途 | 上方动画或[静态图](docs/media/demo-poster.png) | 无需安装；这是概念演绎 |
...
目前没有安装后的一键示例工作区。示例的手动启动只演示业务读写，自动验收才覆盖 Pipedeck 的工作区编排。
...
## 技术架构
![Pipedeck 技术架构：React/Tauri 与 CLI 共用本地控制 API，由规划和执行层协调 Host、Compose 与就绪检查，配置和历史存于 SQLite](docs/media/architecture.svg)
